...
// 👇 PUT YOUR REAL XEN TOKEN ADDRESS HERE
pub const REAL_XEN_MINT: Pubkey =
    pubkey!("A3YKD7vkBjsxKQEWMRLbTkstWyjLENudgAzt2kiumTHR");
...
// Network throttle: 1M XEN per minute globally
...
pub mod xen_time_mint {
    ...
    user_stats.user = ctx.accounts.user.key();
        ...
        require!(ctx.accounts.xen_mint.decimals == 9, XenError::InvalidDecimals);
        require_keys_eq!(
    ctx.accounts.xen_mint.key(),
    REAL_XEN_MINT,
    XenError::InvalidMint
);
...
require_keys_eq!(
    ctx.accounts.vault_xen_ata.key(),
    real_vault_ata,
    XenError::InvalidVault
);
...
require_keys_eq!(
    ctx.accounts.user_xen_ata.key(),
    real_user_ata,
    XenError::InvalidUserATA
);
...
let network_max = if minutes_elapsed == 0 {
    ...
    NETWORK_PER_MINUTE
        ...
        .ok_or(XenError::MathOverflow)?
};

        require!(
            global.total_minted + TAP_AMOUNT <= network_max,
            XenError::RateLimited
        );
        ...
        let mut price = START_PRICE.checked_add(steps).ok_or(XenError::MathOverflow)?;
        if price > MAX_PRICE {
            price = MAX_PRICE;
        }
...
require!(
    ctx.accounts.vault_xen_ata.amount >= TAP_AMOUNT,
    XenError::VaultEmpty
);

        // 2. TRANSFER XEN FROM VAULT (Tap Claim)
        token_interface::transfer_checked(

            CpiContext::new_with_signer(
                ctx.accounts.token_program.to_account_info(),
                TransferChecked {
                    from: ctx.accounts.vault_xen_ata.to_account_info(),
                    to: ctx.accounts.user_xen_ata.to_account_info(),
                    mint: ctx.accounts.xen_mint.to_account_info(),
...
pub struct TapClaim<'info> {
    ...
    pub xen_mint: InterfaceAccount<'info, Mint>,
    ...
    pub vault_xen_ata: InterfaceAccount<'info, TokenAccount>,
    ...
    pub user_xen_ata: InterfaceAccount<'info, TokenAccount>,
